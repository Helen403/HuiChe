...
public class OnLineUnCancelAdapter extends BaseAdapter{
	private List<OnLineUnSendInfo>dataList=new ArrayList<OnLineUnSendInfo>();
	private Context mContext;
	public OnLineUnCancelAdapter(Context mContext,List<OnLineUnSendInfo>dataList){
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		if(convertView==null){
			convertView=LayoutInflater.from(mContext).inflate(R.layout.item_online_hascancel, null);
			holder=new Holder();
		...
		if(data.size()>0){
			...
			OnLineHasCancelProductAdapter adapter=new OnLineHasCancelProductAdapter(mContext, data);
