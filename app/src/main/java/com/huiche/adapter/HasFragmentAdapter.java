...
public class HasFragmentAdapter extends BaseAdapter {
    private List<OnLineUnSendInfo> dataList = new ArrayList<OnLineUnSendInfo>();
    private Context mContext;

    public HasFragmentAdapter(Context mContext, List<OnLineUnSendInfo> dataList) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        if (convertView == null) {
            convertView = LayoutInflater.from(mContext).inflate(R.layout.item_hasfragment, null);
            holder = new Holder();
        ...
        if (data.size() > 0) {
            ...
            HasFragmentProductAdapter adapter = new HasFragmentProductAdapter(mContext, data);
