 ...
 */
public class UoneUActivity extends Activity implements OnClickListener {
    ...
    private int randowWidth;//随机图片的宽度
    private int randowHeight;//随机图片的高度
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 设置沉浸式状态栏
        SystemBarUtil.initSystemBar(this, R.color.deep_blue_bg);
        findView();
        initData();
        setListener();
    }
    ...
    private void findView() {
        civUUIcon = (CircleImageView) findViewById(R.id.civ_uuIcon);
        whewView_U_U = (WhewView) findViewById(R.id.whewView_U_U);
        imb_titleBack_UU = (ImageButton) findViewById(R.id.imb_titleBack_UU);
        tv_titleText_UU = (TextView) findViewById(R.id.tv_titleText_UU);
        rl_randomView_UU = (RelativeLayout) findViewById(R.id.rl_randomView_UU);
        rl_mainContent_UU = (RelativeLayout) findViewById(R.id.rl_mainContent_UU);
        // 设置显示比例为屏幕宽度1/3
        ...
        civUUIcon.setImageResource(R.drawable.shangj_beij11);
        whewView_U_U.setCenterRadius(iconWidth / 2);
        whewView_U_U.setMaxWidth(screenHeight);
        whewView_U_U.setSpeed(4);
    ...
    private void setListener() {
        ...
        imb_titleBack_UU.setOnClickListener(this);
...
//		civUUIcon.setImageResource(R.drawable.bb);
    ...
    /**
     * @param count 随机获取屏幕一点，随机显示图片
     */
    private void getRandomPoint2(int count) {
        randowWidth = screenWidth / 6;//屏幕宽度1/6
        randowHeight = screenWidth / 6;//屏幕宽度1/6
        int startX = 0;
        int startY = 0;
        pointAllList.clear();
        pointList.clear();
        ...
        for (int i = 0; i < screenWidth / randowWidth; i++) {
            for (int j = 0; j < screenHeight / randowHeight; j++) {
                if (startX < screenWidth && startY < screenHeight) {
                    //不在中心圆内才添加，防止覆盖中心图片
                    if (startX + randowWidth < civX || startX > civX + iconWidth
                            || startY + randowHeight < civY || startY > civY + iconHeight) {
        ...
        for (int i = 0; i < imgRes.length; i++) {
            RoundImageView randomView = new RoundImageView(mContext);
            // 设置显示比例
            LayoutParams params = new LayoutParams(randowWidth, randowHeight);
            // 设置显示位置
            params.setMargins(pointList.get(i).x, pointList.get(i).y, 0, 0);
            randomView.setBorderThickness(2);
            randomView.setBorderOutsideColor(getResources().getColor(
