...
import com.huiche.R;
import com.huiche.adapter.ShoppCartAdapter_Test;
 ...
 */
public class ShoppingCartActivityTest extends BaseActivity implements
        ShoppCartAdapter_Test.CheckInterface, ShoppCartAdapter_Test.ModifyCountInterface, OnClickListener {
    ...
    private ShoppCartAdapter_Test adapter;
...
//                            shopingCartIdList.add(child.get(j).getProductP().getId());
    ...
    //结算
    private void buyOverShoppCartProduct(List<String> shopingCartIdList) {
...
//                Log.i("shopCart", response.toString());
    ...
    private void setProductData() {
        if (businessStoreList.size() > 0) {
            ...
            adapter = new ShoppCartAdapter_Test(mContext, businessStoreList, childList);
...
//        ShoppCartAdapter_Test.ViewHolderGroup viewHolderGroup= (ShoppCartAdapter_Test.ViewHolderGroup) adapter.getGroup(groupPosition);
//        ShoppCartAdapter_Test.ViewHolderChild viewHolderChild = (ShoppCartAdapter_Test.ViewHolderChild) adapter.getChild(groupPosition, childPosition);
//        childList.get(businessStoreList.get(groupPosition).getId()).get(childPosition).setIsChoseProduct(isChecked);
